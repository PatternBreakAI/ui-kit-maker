...
| `/api/export` | POST | Reads `plan_id` from the database and either refuses or issues an export grant (licence block + reference), logging it for the rate limit. The browser will not assemble a paid artifact without one. Gate Round (2026-08-17): only `student`/`pro` are ever granted — the free plan's set is empty; its 403 points at Pro and the free test kit. |
| `/api/test-kit` | POST | The registered tier's one download: the stock Unity **free kit** — a single admin-blessed zip in the private `test-kit` bucket (fixed artifact, never the caller's design; its ship-anything free-kit licence rides inside the zip). Verifies the caller is a signed-in user (guests → 401 with `reason:"signin"`, the register incentive), then mints a ~10-minute signed download URL. Not stocked yet → honest 404. Logs a best-effort `export_events` row (`kind:"testkit"`). |
...
| `/api/assets` | POST | The quota gate for durable backdrop storage (v94). `{action:"grant", hash, size, type}` verifies the caller's token, reads their `plan_id` (free 50 MB, paid 1 GB), sums what is actually in their `bg-assets` folder, and either refuses (413) or mints a **signed upload URL** for `<uid>/<sha-256 prefix>` — the bytes go browser→storage directly, never through the function. The bucket has **no insert policy**, so this grant is the only way in; the bucket's 8 MB `file_size_limit` backstops a lying `size`. `{action:"usage"}` returns `{used, quota}`. |
| `/api/admin` | POST | Admin-only, same gate: the CALLER's own profile row must say `is_admin` (read with the service role — nothing the client claims is believed). `{action:"search", q}` finds accounts by email substring (Stripe ids stay server-side; the client only learns present/absent). `{action:"setPlan", userId, plan}` comps or revokes: grants stamp `plan_status='comped'`, revokes `'canceled'`, so a comp is never mistaken for a Stripe purchase. `{action:"testKitStatus"}` / `{action:"testKitUpload", size}` stock and swap the blessed Unity free-kit zip — upload mints a signed URL so the bytes go browser→storage, and the `test-kit` bucket is created lazily on first use. Every change is audit-logged (structured console line + best-effort `admin_audit` insert). Serves the `#/admin` page (linked only from the Account page's admin-only card). |
