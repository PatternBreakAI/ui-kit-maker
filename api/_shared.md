...
| `/api/test-kit` | POST | The registered tier's one download: the stock Unity **evaluation kit** — a single admin-blessed zip in the private `test-kit` bucket (fixed artifact, never the caller's design). Verifies the caller is a signed-in user (guests → 401 with `reason:"signin"`, the register incentive), then mints a ~10-minute signed download URL. Not stocked yet → honest 404. Logs a best-effort `export_events` row (`kind:"testkit"`). |
