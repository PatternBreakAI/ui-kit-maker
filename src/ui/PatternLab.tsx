...
function Swatch(props: { id: string; w: number; h: number; ps: number; color: string; angle: number; bg: string; radius?: number }) {
  const { id, w, h, ps, color, angle, bg, radius } = props;
  const pid = `lab-${id}-${Math.round(ps)}-${angle}`;
...
export function PatternLab() {
  ...
  const [context, setContext] = useState<"swatch" | "seam" | "components">("swatch");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#12141C", color: "#E9EDF7", fontFamily: "Inter, system-ui, sans-serif", padding: 28 }}>
      ...
      <div style={{ display: "flex", gap: 18, alignItems: "center", marginBottom: 20, flexWrap: "wrap", fontSize: 13 }}>
        ...
        <label>View{" "}
          {(["swatch", "seam", "components"] as const).map((v) => (
