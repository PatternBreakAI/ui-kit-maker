      ...
      <Section id="silhouette" title={t("secSilhouette")} summary={<span>{SHAPES.find((sh) => sh.id === D.shape)?.name.split(" — ")[0]}</span>}>
        ...
        <div className="silcats" role="radiogroup" aria-label="Silhouette category">
          ...
          {["All", ...SILHOUETTE_CATEGORIES.filter((cat) =>
            SILHOUETTES.some((m) => m.category === cat && (!m.preview || isAdmin || D.shape === m.id)),
          )].map((cat) => (
        ...
        <div className="shapegrid">
          {SILHOUETTES
            ...
            .filter((m) => !m.preview || isAdmin || D.shape === m.id)
            ...
            <button key={m.id} className={`shapecard${(focus ? (kitShapes[focus] ?? KIT_SHAPE[focus] ?? D.shape) : D.shape) === m.id ? " on" : ""}${retired ? " retired" : ""}`}
              title={retired ? `${m.name} — retired from the picker` : `${m.name} — ${m.character}${m.preview ? " (unlisted preview — admins only)" : ""}`}
      ...
      {outlines && (
        <div className="devoutlines" role="dialog" aria-label="Silhouette outline comparison" onClick={() => setOutlines(false)}>
          ...
          {SILHOUETTE_CATEGORIES
            ...
            .filter((cat) => SILHOUETTES.some((m) => m.category === cat && (!m.preview || isAdmin)))
            .map((cat) => (
          <div key={cat}>
          <div className="devo-cat">{cat}</div>
          <div className="devo-grid">
            {SILHOUETTES.filter((m) => m.category === cat && (!m.preview || isAdmin)).map((m) => {
