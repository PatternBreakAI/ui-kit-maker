...
cust:"CUSTOMIZE",pushKit:"CREATE YOUR KIT",pushBoard:"PUSH TO A BOARD",pushExport:"EXPORT",pushOpen:"OPEN THE GENERATOR",shipDone:"EXPORT COMPLETE",shipLine:"Yours to ship — in any game or product you sell.*",compatLbl:"Plays nice with your stack",compatTitle:"Lands in the tools you already use.",c_rest:"Roll over a logo — here's how your kit lands there.",c_svg:"Clean SVG vectors import as fully editable paths — every component and state, named and grouped.",c_png:"Crisp transparent PNGs at 1× and 2×, cut per component and state.",c_html:"A live kit.html + kit.css — open it, inspect it, or lift the styles wholesale.",c_json:"An engine-ready JSON manifest that maps every piece, state, and size.",c_fold:"A tidy, predictable folder structure that drops straight into your project.",c_soon:"NATIVE EXPORT ON THE ROADMAP",c_unity:"Unity has no native SVG runtime — so your kit lands as a crisp PNG sprite atlas with 9-slice borders and an engine-ready JSON manifest, straight into UGUI or UI Toolkit.",c_unreal:"UMG doesn't speak SVG — you get texture-named PNGs plus Slate brush margins in JSON, ready to wire into Widget Blueprints without renaming a thing.",c_godot:"Godot 4 imports SVG directly (the static SVG 1.1 profile, rendered by ThorVG) — drop the vectors in and they stay sharp at any scale, with PNG fallbacks included.",c_gm:"GameMaker takes sprites, not vectors — your kit arrives as clean 1× and 2× PNGs, cut per component and state, ready for sprite slots and nine-slices.",c_c3:"Use the SVG Picture object (static SVG 1.1) for infinite-resolution UI, or the per-state PNGs for classic sprites — both are in the box.",c_rblx:"Roblox UI is PNG territory — transparent per-state slices sized for ImageLabels and ImageButtons, with 9-slice insets that match SliceCenter.",c_rpgm:"Skins and sheets, ready to drop in — the kit exports PNG pieces you can compose straight into window skins and system sheets.",c_ase:"Open the 1× PNGs and retouch them pixel by pixel — every component and state imports on its own canvas.",c_figma:"SVG pastes straight in as fully editable vectors — every piece, every layer. Recreate anything custom with ease, and build the common pieces in our app first so you only fine-tune in Figma.",c_penpot:"Penpot is SVG-native — our vectors don't get converted, they just become your file. Every path, gradient, and group stays exactly as exported.",c_sketch:"Sketch imports SVG 1.1 as editable layers — components arrive as named groups with live paths and gradients you can restyle in place.",c_aff:"Affinity Designer opens SVG 1.1 with curves intact — every component fully editable, plus per-state PNGs when you just need placement.",c_ps:"Vectors aren't Photoshop's thing — so you get layered, transparent PNGs at 1× and 2×, cut per component and state, perfect for smart objects and mockups.",c_ai:"Illustrator reads SVG 1.1 natively — paths, gradients, and groups land fully editable, named per component and state.",c_ae:"AE takes the per-state PNGs straight into motion; for true shape layers, route the SVGs through Illustrator and they convert cleanly.",c_blender:"Blender imports SVG 1.1 as curve objects — extrude your UI into diegetic 3D panels, or texture with the crisp PNGs.",c_krita:"Krita opens SVG 1.1 on vector layers — edit paths in place, or paint over the PNG exports.",c_web:"The web is home turf — inline SVG plus a live kit.html and kit.css you can open, inspect, or lift wholesale.",ks1:"BUTTONS",ks2:"INTERFACE",ks3:"CONTROLS",ks4:"METERS & TIMERS",ks5:"HUD & PLAYER",ks6:"BIG PIECES",ks7:"NEW FROM THE ENGINE",
...
cust:"自定义",pushKit:"创建你的组件库",pushBoard:"进入画板",pushExport:"导出",pushOpen:"打开生成器",shipDone:"导出完成",shipLine:"归你所有 — 可用于任何你销售的游戏或产品。*",compatLbl:"兼容你的工作流",compatTitle:"直接落地到你常用的工具。",c_rest:"将鼠标悬停在图标上,查看套件如何进入该工具。",c_svg:"干净的 SVG 矢量可直接导入并完全编辑 — 每个组件和状态都已命名分组。",c_png:"清晰的透明 PNG(1× 与 2×),按组件和状态切分。",c_html:"附带可运行的 kit.html 与 kit.css — 可直接打开、检查或整体套用样式。",c_json:"引擎可用的 JSON 清单,映射每个组件、状态和尺寸。",c_fold:"整洁可预期的文件夹结构,可直接放入项目。",c_soon:"原生导出即将推出",c_unity:"Unity 没有原生 SVG 运行时——套件以清晰的 PNG 精灵图集落地,附 9 宫格边距和引擎可用的 JSON 清单,直接进入 UGUI 或 UI Toolkit。",c_unreal:"UMG 不支持 SVG——你会得到按贴图规范命名的 PNG,以及 JSON 里的 Slate 笔刷边距,接入 Widget 蓝图无需改名。",c_godot:"Godot 4 可直接导入 SVG(静态 SVG 1.1,由 ThorVG 渲染)——矢量放入后任意缩放都清晰,并附 PNG 备用。",c_gm:"GameMaker 用的是精灵而非矢量——套件以干净的 1× 和 2× PNG 到达,按组件和状态切分,可直接用于精灵槽位和九宫格。",c_c3:"用 SVG Picture 对象(静态 SVG 1.1)获得无限分辨率的 UI,或用按状态切分的 PNG 做经典精灵——两者都在包里。",c_rblx:"Roblox 的 UI 是 PNG 的领域——透明的按状态切片,适配 ImageLabel 和 ImageButton,9 宫格内边距与 SliceCenter 对应。",c_rpgm:"即放即用的皮肤与素材表——套件导出的 PNG 可直接拼进窗口皮肤和系统素材表。",c_ase:"打开 1× PNG 逐像素修饰——每个组件和状态都在独立画布上导入。",c_figma:"SVG 直接粘贴为完全可编辑的矢量——每个部件、每个图层。轻松重制任何自定义样式;常用组件先在我们的应用里做好,在 Figma 里只需微调。",c_penpot:"Penpot 以 SVG 为原生格式——我们的矢量无需转换,直接成为你的文件。每条路径、渐变和分组都保持原样。",c_sketch:"Sketch 将 SVG 1.1 导入为可编辑图层——组件以命名分组到达,路径和渐变可就地重新设计。",c_aff:"Affinity Designer 打开 SVG 1.1 时曲线完整——每个组件完全可编辑,另附按状态的 PNG 便于直接摆放。",c_ps:"矢量不是 Photoshop 的强项——所以你会得到分层透明 PNG(1× 与 2×),按组件和状态切分,适合智能对象和样机。",c_ai:"Illustrator 原生读取 SVG 1.1——路径、渐变和分组完全可编辑,并按组件和状态命名。",c_ae:"AE 可直接用按状态的 PNG 做动效;需要真正的形状图层时,经 Illustrator 转换 SVG 即可。",c_blender:"Blender 将 SVG 1.1 导入为曲线对象——把 UI 挤出成 3D 面板,或用清晰的 PNG 作贴图。",c_krita:"Krita 在矢量图层上打开 SVG 1.1——就地编辑路径,或在 PNG 导出上作画。",c_web:"Web 是主场——内联 SVG,加上可运行的 kit.html 与 kit.css,可打开、检查或整体套用。",ks1:"按钮",ks2:"界面",ks3:"控件",ks4:"仪表与计时",ks5:"HUD与玩家",ks6:"大型组件",ks7:"引擎新组件",
...
cust:"PERSONNALISER",pushKit:"CRÉEZ VOTRE KIT",pushBoard:"VERS LE BOARD",pushExport:"EXPORTER",pushOpen:"OUVRIR LE GÉNÉRATEUR",shipDone:"EXPORT TERMINÉ",shipLine:"À vous — dans tout jeu ou produit que vous vendez.*",compatLbl:"Compatible avec vos outils",compatTitle:"Atterrit dans les outils que vous utilisez déjà.",c_rest:"Survolez un logo — voici comment votre kit y atterrit.",c_svg:"Des vecteurs SVG propres, entièrement éditables — chaque composant et état, nommé et groupé.",c_png:"Des PNG transparents et nets en 1× et 2×, découpés par composant et par état.",c_html:"Un kit.html + kit.css vivants — à ouvrir, inspecter ou réutiliser tels quels.",c_json:"Un manifeste JSON prêt pour le moteur, qui répertorie chaque pièce, état et taille.",c_fold:"Une arborescence propre et prévisible, à glisser directement dans votre projet.",c_soon:"EXPORT NATIF SUR LA ROADMAP",c_unity:"Unity n'a pas de runtime SVG natif — votre kit arrive en atlas de sprites PNG nets, avec marges 9-slice et manifeste JSON prêt pour le moteur, direct dans UGUI ou UI Toolkit.",c_unreal:"UMG ne parle pas SVG — vous recevez des PNG nommés façon textures et les marges de brush Slate en JSON, à brancher dans vos Widget Blueprints sans rien renommer.",c_godot:"Godot 4 importe le SVG directement (profil statique SVG 1.1, rendu par ThorVG) — vos vecteurs restent nets à toute échelle, PNG de secours inclus.",c_gm:"GameMaker veut des sprites, pas des vecteurs — le kit arrive en PNG propres 1× et 2×, découpés par composant et état, prêts pour les slots de sprites et le nine-slice.",c_c3:"Utilisez l'objet SVG Picture (SVG 1.1 statique) pour une UI en résolution infinie, ou les PNG par état pour des sprites classiques — les deux sont fournis.",c_rblx:"L'UI Roblox, c'est du PNG — des découpes transparentes par état, dimensionnées pour ImageLabel et ImageButton, avec des marges 9-slice alignées sur SliceCenter.",c_rpgm:"Skins et planches prêtes à poser — le kit exporte des pièces PNG à composer directement en window skins et planches système.",c_ase:"Ouvrez les PNG 1× et retouchez pixel par pixel — chaque composant et état s'importe sur son propre canevas.",c_figma:"Le SVG se colle en vecteurs entièrement éditables — chaque pièce, chaque calque. Recréez n'importe quel élément custom avec aisance, et créez les pièces courantes dans notre app pour ne faire que peaufiner dans Figma.",c_penpot:"Penpot est SVG natif — nos vecteurs ne sont pas convertis, ils deviennent votre fichier. Chaque tracé, dégradé et groupe reste exactement tel quel.",c_sketch:"Sketch importe le SVG 1.1 en calques éditables — les composants arrivent en groupes nommés, tracés et dégradés modifiables sur place.",c_aff:"Affinity Designer ouvre le SVG 1.1 avec les courbes intactes — chaque composant reste éditable, plus des PNG par état pour la simple mise en place.",c_ps:"Le vectoriel n'est pas le fort de Photoshop — vous recevez donc des PNG transparents en calques, 1× et 2×, découpés par composant et état, parfaits pour objets dynamiques et maquettes.",c_ai:"Illustrator lit le SVG 1.1 nativement — tracés, dégradés et groupes arrivent entièrement éditables, nommés par composant et état.",c_ae:"AE anime directement les PNG par état ; pour de vrais calques de forme, passez les SVG par Illustrator, la conversion est propre.",c_blender:"Blender importe le SVG 1.1 en courbes — extrudez votre UI en panneaux 3D diégétiques, ou texturez avec les PNG nets.",c_krita:"Krita ouvre le SVG 1.1 sur des calques vectoriels — éditez les tracés sur place, ou peignez par-dessus les exports PNG.",c_web:"Le web, c'est la maison — SVG inline plus un kit.html et kit.css vivants, à ouvrir, inspecter ou reprendre tels quels.",ks1:"BOUTONS",ks2:"INTERFACE",ks3:"CONTRÔLES",ks4:"JAUGES & CHRONOS",ks5:"HUD & JOUEUR",ks6:"GRANDES PIÈCES",ks7:"NOUVEAUTÉS DU MOTEUR",
...
cust:"PERSONALIZAR",pushKit:"CREA TU KIT",pushBoard:"AL BOARD",pushExport:"EXPORTAR",pushOpen:"ABRIR EL GENERADOR",shipDone:"EXPORTACIÓN COMPLETA",shipLine:"Tuyo — en cualquier juego o producto que vendas.*",compatLbl:"Compatible con tus herramientas",compatTitle:"Aterriza en las herramientas que ya usas.",c_rest:"Pasa el cursor por un logo — así llega tu kit.",c_svg:"Vectores SVG limpios y totalmente editables — cada componente y estado, nombrado y agrupado.",c_png:"PNG transparentes y nítidos en 1× y 2×, cortados por componente y estado.",c_html:"Un kit.html + kit.css vivos — ábrelo, inspecciónalo o reutiliza los estilos.",c_json:"Un manifiesto JSON listo para el motor que mapea cada pieza, estado y tamaño.",c_fold:"Una estructura de carpetas limpia y predecible que entra directa en tu proyecto.",c_soon:"EXPORTACIÓN NATIVA EN CAMINO",c_unity:"Unity no tiene runtime SVG nativo — tu kit aterriza como atlas de sprites PNG nítidos, con bordes 9-slice y un manifiesto JSON listo para el motor, directo a UGUI o UI Toolkit.",c_unreal:"UMG no habla SVG — recibes PNG con nombres de textura y los márgenes de brush de Slate en JSON, listos para conectar a tus Widget Blueprints sin renombrar nada.",c_godot:"Godot 4 importa SVG directamente (perfil estático SVG 1.1, renderizado por ThorVG) — tus vectores se ven nítidos a cualquier escala, con PNG de respaldo incluidos.",c_gm:"GameMaker usa sprites, no vectores — el kit llega en PNG limpios a 1× y 2×, cortados por componente y estado, listos para slots de sprites y nine-slice.",c_c3:"Usa el objeto SVG Picture (SVG 1.1 estático) para UI de resolución infinita, o los PNG por estado para sprites clásicos — ambos vienen en la caja.",c_rblx:"La UI de Roblox es territorio PNG — recortes transparentes por estado, a medida de ImageLabel e ImageButton, con márgenes 9-slice que casan con SliceCenter.",c_rpgm:"Skins y hojas listas para colocar — el kit exporta piezas PNG que puedes componer directamente en window skins y hojas de sistema.",c_ase:"Abre los PNG a 1× y retócalos píxel a píxel — cada componente y estado se importa en su propio lienzo.",c_figma:"El SVG se pega como vectores totalmente editables — cada pieza, cada capa. Recrea cualquier elemento custom con facilidad, y crea las piezas comunes en nuestra app para solo afinar en Figma.",c_penpot:"Penpot es SVG nativo — nuestros vectores no se convierten: se vuelven tu archivo. Cada trazado, degradado y grupo queda exactamente igual.",c_sketch:"Sketch importa SVG 1.1 como capas editables — los componentes llegan en grupos con nombre, con trazados y degradados que puedes reestilizar al momento.",c_aff:"Affinity Designer abre SVG 1.1 con las curvas intactas — cada componente totalmente editable, más PNG por estado para cuando solo necesitas colocar.",c_ps:"Lo vectorial no es lo de Photoshop — así que recibes PNG transparentes por capas a 1× y 2×, cortados por componente y estado, perfectos para objetos inteligentes y mockups.",c_ai:"Illustrator lee SVG 1.1 de forma nativa — trazados, degradados y grupos llegan totalmente editables, con nombre por componente y estado.",c_ae:"AE lleva los PNG por estado directo a la animación; para capas de forma reales, pasa los SVG por Illustrator y la conversión sale limpia.",c_blender:"Blender importa SVG 1.1 como curvas — extruye tu UI en paneles 3D diegéticos, o texturiza con los PNG nítidos.",c_krita:"Krita abre SVG 1.1 en capas vectoriales — edita los trazados ahí mismo, o pinta sobre los PNG exportados.",c_web:"La web es casa — SVG inline más un kit.html y kit.css vivos que puedes abrir, inspeccionar o aprovechar enteros.",ks1:"BOTONES",ks2:"INTERFAZ",ks3:"CONTROLES",ks4:"MEDIDORES Y TIEMPO",ks5:"HUD Y JUGADOR",ks6:"PIEZAS GRANDES",ks7:"NUEVO DEL MOTOR",
...
cust:"PERSONALIZZA",pushKit:"CREA IL TUO KIT",pushBoard:"ALLA BOARD",pushExport:"ESPORTA",pushOpen:"APRI IL GENERATORE",shipDone:"EXPORT COMPLETATO",shipLine:"Tuo — in qualsiasi gioco o prodotto che vendi.*",compatLbl:"Compatibile con i tuoi strumenti",compatTitle:"Arriva negli strumenti che già usi.",c_rest:"Passa su un logo — ecco come arriva il tuo kit.",c_svg:"Vettori SVG puliti e completamente modificabili — ogni componente e stato, nominato e raggruppato.",c_png:"PNG trasparenti e nitidi in 1× e 2×, tagliati per componente e stato.",c_html:"Un kit.html + kit.css dal vivo — aprilo, ispezionalo o riusa gli stili.",c_json:"Un manifest JSON pronto per l'engine che mappa ogni pezzo, stato e dimensione.",c_fold:"Una struttura di cartelle ordinata e prevedibile, pronta per il tuo progetto.",c_soon:"EXPORT NATIVO IN ARRIVO",c_unity:"Unity non ha un runtime SVG nativo — il kit arriva come atlas di sprite PNG nitidi, con bordi 9-slice e manifest JSON pronto per l'engine, dritto in UGUI o UI Toolkit.",c_unreal:"UMG non parla SVG — ricevi PNG nominati come texture e i margini brush di Slate in JSON, pronti da collegare ai Widget Blueprint senza rinominare nulla.",c_godot:"Godot 4 importa SVG direttamente (profilo statico SVG 1.1, reso da ThorVG) — i vettori restano nitidi a ogni scala, con PNG di riserva inclusi.",c_gm:"GameMaker vuole sprite, non vettori — il kit arriva in PNG puliti a 1× e 2×, tagliati per componente e stato, pronti per slot sprite e nine-slice.",c_c3:"Usa l'oggetto SVG Picture (SVG 1.1 statico) per UI a risoluzione infinita, o i PNG per stato per gli sprite classici — entrambi inclusi.",c_rblx:"La UI di Roblox è territorio PNG — ritagli trasparenti per stato, su misura per ImageLabel e ImageButton, con margini 9-slice allineati a SliceCenter.",c_rpgm:"Skin e sheet pronti da posare — il kit esporta pezzi PNG da comporre direttamente in window skin e sheet di sistema.",c_ase:"Apri i PNG a 1× e ritoccali pixel per pixel — ogni componente e stato si importa sul proprio canvas.",c_figma:"L'SVG si incolla come vettori completamente modificabili — ogni pezzo, ogni livello. Ricrea qualsiasi elemento custom con facilità, e crea i pezzi comuni nella nostra app per rifinire soltanto in Figma.",c_penpot:"Penpot è SVG nativo — i nostri vettori non vengono convertiti: diventano il tuo file. Ogni tracciato, gradiente e gruppo resta esattamente com'è.",c_sketch:"Sketch importa SVG 1.1 come livelli modificabili — i componenti arrivano in gruppi nominati, con tracciati e gradienti da ristilizzare sul posto.",c_aff:"Affinity Designer apre SVG 1.1 con le curve intatte — ogni componente resta modificabile, più PNG per stato quando serve solo posizionare.",c_ps:"Il vettoriale non è il forte di Photoshop — quindi ricevi PNG trasparenti a livelli in 1× e 2×, tagliati per componente e stato, perfetti per oggetti avanzati e mockup.",c_ai:"Illustrator legge SVG 1.1 nativamente — tracciati, gradienti e gruppi arrivano completamente modificabili, nominati per componente e stato.",c_ae:"AE porta i PNG per stato dritti in animazione; per veri shape layer, passa gli SVG da Illustrator: la conversione è pulita.",c_blender:"Blender importa SVG 1.1 come curve — estrudi la tua UI in pannelli 3D diegetici, o texturizza con i PNG nitidi.",c_krita:"Krita apre SVG 1.1 su livelli vettoriali — modifica i tracciati sul posto, o dipingi sopra gli export PNG.",c_web:"Il web è casa — SVG inline più kit.html e kit.css vivi da aprire, ispezionare o riusare in blocco.",ks1:"PULSANTI",ks2:"INTERFACCIA",ks3:"CONTROLLI",ks4:"INDICATORI E TIMER",ks5:"HUD E GIOCATORE",ks6:"PEZZI GRANDI",ks7:"NOVITÀ DAL MOTORE",
...
cust:"ANPASSEN",pushKit:"ERSTELLE DEIN KIT",pushBoard:"AUFS BOARD",pushExport:"EXPORTIEREN",pushOpen:"GENERATOR ÖFFNEN",shipDone:"EXPORT ABGESCHLOSSEN",shipLine:"Gehört dir — in jedem Spiel oder Produkt, das du verkaufst.*",compatLbl:"Spielt mit deinem Stack",compatTitle:"Landet in den Tools, die du schon nutzt.",c_rest:"Fahre über ein Logo — so landet dein Kit dort.",c_svg:"Saubere SVG-Vektoren, voll editierbar — jede Komponente und jeder State, benannt und gruppiert.",c_png:"Gestochen scharfe transparente PNGs in 1× und 2×, geschnitten pro Komponente und State.",c_html:"Ein lebendes kit.html + kit.css — öffnen, inspizieren oder Styles direkt übernehmen.",c_json:"Ein engine-fertiges JSON-Manifest, das jedes Teil, jeden State und jede Größe abbildet.",c_fold:"Eine aufgeräumte, vorhersehbare Ordnerstruktur, die direkt ins Projekt fällt.",c_soon:"NATIVER EXPORT GEPLANT",c_unity:"Unity hat keine native SVG-Runtime — dein Kit landet als gestochen scharfer PNG-Sprite-Atlas mit 9-Slice-Rändern und engine-fertigem JSON-Manifest, direkt in UGUI oder UI Toolkit.",c_unreal:"UMG spricht kein SVG — du bekommst texture-konform benannte PNGs und die Slate-Brush-Ränder als JSON, bereit für Widget Blueprints, ohne etwas umzubenennen.",c_godot:"Godot 4 importiert SVG direkt (statisches SVG-1.1-Profil, gerendert von ThorVG) — Vektoren bleiben in jeder Skalierung scharf, PNG-Fallbacks inklusive.",c_gm:"GameMaker will Sprites, keine Vektoren — das Kit kommt als saubere PNGs in 1× und 2×, geschnitten pro Komponente und State, bereit für Sprite-Slots und Nine-Slice.",c_c3:"Nutz das SVG-Picture-Objekt (statisches SVG 1.1) für UI in unendlicher Auflösung, oder die PNGs pro State für klassische Sprites — beides liegt bei.",c_rblx:"Roblox-UI ist PNG-Gebiet — transparente Schnitte pro State, passend für ImageLabel und ImageButton, mit 9-Slice-Rändern, die zu SliceCenter passen.",c_rpgm:"Skins und Sheets zum Direkteinsetzen — das Kit exportiert PNG-Teile, die du direkt zu Window Skins und System-Sheets zusammensetzt.",c_ase:"Öffne die 1×-PNGs und arbeite Pixel für Pixel — jede Komponente und jeder State importiert auf eigener Leinwand.",c_figma:"SVG landet als voll editierbare Vektoren — jedes Teil, jede Ebene. Baue jedes Custom-Piece mühelos nach, und erstelle die gängigen Teile erst in unserer App, sodass du in Figma nur noch feinschleifst.",c_penpot:"Penpot ist SVG-nativ — unsere Vektoren werden nicht konvertiert, sie werden deine Datei. Jeder Pfad, Verlauf und jede Gruppe bleibt exakt erhalten.",c_sketch:"Sketch importiert SVG 1.1 als editierbare Ebenen — Komponenten kommen als benannte Gruppen mit Pfaden und Verläufen, die du direkt umstylst.",c_aff:"Affinity Designer öffnet SVG 1.1 mit intakten Kurven — jede Komponente voll editierbar, plus PNGs pro State, wenn du nur platzieren willst.",c_ps:"Vektoren sind nicht Photoshops Ding — dafür gibt's transparente, ebenenfertige PNGs in 1× und 2×, geschnitten pro Komponente und State, perfekt für Smartobjekte und Mockups.",c_ai:"Illustrator liest SVG 1.1 nativ — Pfade, Verläufe und Gruppen kommen voll editierbar an, benannt pro Komponente und State.",c_ae:"AE nimmt die PNGs pro State direkt in die Animation; für echte Shape-Layer schickst du die SVGs durch Illustrator — die Umwandlung ist sauber.",c_blender:"Blender importiert SVG 1.1 als Kurven — extrudiere deine UI zu diegetischen 3D-Panels, oder texturiere mit den scharfen PNGs.",c_krita:"Krita öffnet SVG 1.1 auf Vektorebenen — bearbeite Pfade direkt, oder mal über die PNG-Exporte.",c_web:"Das Web ist Heimspiel — Inline-SVG plus lebendes kit.html und kit.css zum Öffnen, Inspizieren oder Komplett-Übernehmen.",ks1:"BUTTONS",ks2:"INTERFACE",ks3:"STEUERUNG",ks4:"ANZEIGEN & TIMER",ks5:"HUD & SPIELER",ks6:"GROSSE TEILE",ks7:"NEU AUS DER ENGINE",
...
cust:"カスタマイズ",pushKit:"キットを作成",pushBoard:"ボードへ",pushExport:"書き出す",pushOpen:"ジェネレーターを開く",shipDone:"エクスポート完了",shipLine:"あなたのもの — 販売するあらゆるゲームや製品に。*",compatLbl:"あなたのツールと連携",compatTitle:"いつものツールに、そのまま届く。",c_rest:"ロゴにカーソルを合わせると、キットの取り込み方法が表示されます。",c_svg:"クリーンな SVG ベクターを完全編集可能なまま読み込み — 全コンポーネント・全ステートが命名済み。",c_png:"透過 PNG を 1×/2× で書き出し。コンポーネント・ステートごとに分割済み。",c_html:"動く kit.html + kit.css 付き。開いて確認、そのままスタイルの流用も。",c_json:"全パーツ・ステート・サイズを網羅したエンジン対応 JSON マニフェスト。",c_fold:"整理されたフォルダ構造で、プロジェクトへそのまま投入可能。",c_soon:"ネイティブ書き出しは今後対応",c_unity:"Unity にネイティブの SVG ランタイムはありません — キットはくっきりした PNG スプライトアトラスとして届きます。9スライス境界とエンジン対応 JSON マニフェスト付きで、UGUI や UI Toolkit にそのまま。",c_unreal:"UMG は SVG を扱えません — テクスチャ命名規則の PNG と Slate ブラシのマージン(JSON)が届き、リネームなしで Widget Blueprint に接続できます。",c_godot:"Godot 4 は SVG を直接インポート(静的 SVG 1.1 プロファイル、ThorVG レンダリング)— どのスケールでもシャープ。PNG の予備も同梱。",c_gm:"GameMaker はベクターではなくスプライト — キットは 1×/2× のクリーンな PNG で届き、コンポーネント・ステートごとに分割済み。スプライトスロットと 9 スライスにそのまま。",c_c3:"SVG Picture オブジェクト(静的 SVG 1.1)で無限解像度の UI に、またはステートごとの PNG でクラシックなスプライトに — 両方入っています。",c_rblx:"Roblox の UI は PNG の世界 — ImageLabel / ImageButton 向けサイズの透過スライスと、SliceCenter に対応する 9 スライスのインセット付き。",c_rpgm:"置くだけのスキンとシート — キットは PNG パーツを書き出し、ウィンドウスキンやシステムシートにそのまま組み込めます。",c_ase:"1× の PNG を開いてピクセル単位でリタッチ — 各コンポーネント・ステートが独立したキャンバスで届きます。",c_figma:"SVG は完全編集可能なベクターとして貼り付け — 全パーツ・全レイヤー。どんなカスタムも簡単に再現でき、定番パーツは先にアプリで作って Figma では微調整だけ。",c_penpot:"Penpot は SVG ネイティブ — ベクターは変換されず、そのままあなたのファイルに。パス・グラデーション・グループはすべて元のまま。",c_sketch:"Sketch は SVG 1.1 を編集可能なレイヤーとして読み込み — コンポーネントは命名済みグループで届き、パスもグラデーションもその場で再スタイルできます。",c_aff:"Affinity Designer は SVG 1.1 をカーブそのままに開きます — 全コンポーネントが編集可能。配置だけならステートごとの PNG も。",c_ps:"ベクターは Photoshop の領分ではないので — 1×/2× の透過レイヤー PNG をコンポーネント・ステートごとに用意。スマートオブジェクトやモックアップに最適。",c_ai:"Illustrator は SVG 1.1 をネイティブに読み込み — パス・グラデーション・グループが完全編集可能なまま、コンポーネントとステートごとに命名されて届きます。",c_ae:"AE はステートごとの PNG をそのままモーションへ。本物のシェイプレイヤーが必要なら、SVG を Illustrator 経由で変換すればきれいに移行できます。",c_blender:"Blender は SVG 1.1 をカーブとしてインポート — UI を押し出して 3D パネルに、または鮮明な PNG をテクスチャに。",c_krita:"Krita は SVG 1.1 をベクターレイヤーで開きます — パスをその場で編集、または PNG 書き出しの上にペイント。",c_web:"Web はホームグラウンド — インライン SVG に、動く kit.html と kit.css。開いて、調べて、丸ごと流用も。",ks1:"ボタン",ks2:"インターフェース",ks3:"コントロール",ks4:"メーターとタイマー",ks5:"HUDとプレイヤー",ks6:"ラージピース",ks7:"エンジンの新パーツ",
...
auT1:"おかえりなさい",auT2:"アカウントを作成",auIn:"サインイン",auUp:"アカウント作成",auEmail:"メール",auPass:"パスワード",auFgt:"パスワードをお忘れですか？",auGo1:"サインイン",auGo2:"アカウントを作成",auOr:"または",auMagic:"✉ サインインリンクを送る",auFreeL:"Free Explorer——カード不要。",auTerms:"利用規約とライセンスに同意します：どんな製品にも使えますが、素材の転売は不可。",auHi:"プレイヤー1",auOkT:"サインインしました",auOkP:"スタジオを開いています：",auDoneT:"アカウントを作成しました",auDoneP:"ようこそ——エディタを開いています：",auSentT2:"受信トレイをご確認ください",auSentP2:"サインインリンクを送信しました：",auRstT:"リセットリンクを送信しました",auRstP:"パスワード再設定の案内を送信しました：",auBackL:"戻る",bgsL:"背景"}
      ...
      try {
        ...
        const CTOOLS = [
          ["Unity", "#9aa3c7", ["p", CP.unity, "mono"], "c_unity", 1],
