...
cust:"カスタマイズ",pushKit:"キットを作成",pushBoard:"ボードへ",pushExport:"書き出す",pushOpen:"ジェネレーターを開く",shipDone:"エクスポート完了",shipLine:"あなたのもの — 販売するあらゆるゲームや製品に。*",compatLbl:"あなたのツールと連携",compatTitle:"いつものツールに、そのまま届く。",c_rest:"ロゴにカーソルを合わせると、キットの取り込み方法が表示されます。",c_svg:"クリーンな SVG ベクターを完全編集可能なまま読み込み — 全コンポーネント・全ステートが命名済み。",c_png:"透過 PNG を 1×/2× で書き出し。コンポーネント・ステートごとに分割済み。",c_html:"動く kit.html + kit.css 付き。開いて確認、そのままスタイルの流用も。",c_json:"全パーツ・ステート・サイズを網羅したエンジン対応 JSON マニフェスト。",c_fold:"整理されたフォルダ構造で、プロジェクトへそのまま投入可能。",c_soon:"ネイティブ書き出しは今後対応",c_unity:"Unity にネイティブの SVG ランタイムはありません — キットはくっきりした PNG スプライトアトラスとして届きます。9スライス境界とエンジン対応 JSON マニフェスト付きで、UGUI や UI Toolkit にそのまま。",c_unreal:"UMG は SVG を扱えません — テクスチャ命名規則の PNG と Slate ブラシのマージン(JSON)が届き、リネームなしで Widget Blueprint に接続できます。",c_godot:"Godot 4 は SVG を直接インポート(静的 SVG 1.1 プロファイル、ThorVG レンダリング)— どのスケールでもシャープ。PNG の予備も同梱。",c_gm:"GameMaker はベクターではなくスプライト — キットは最大 4× のクリーンな透過 PNG で届き、コンポーネント・ステートごとに分割済み。スプライトスロットと 9 スライスにそのまま。",c_c3:"SVG Picture オブジェクト(静的 SVG 1.1)で無限解像度の UI に、またはステートごとの PNG でクラシックなスプライトに — 両方入っています。",c_rblx:"Roblox の UI は PNG の世界 — ImageLabel / ImageButton 向けサイズの透過スライスと、SliceCenter に対応する 9 スライスのインセット付き。",c_rpgm:"置くだけのスキンとシート — キットは PNG パーツを書き出し、ウィンドウスキンやシステムシートにそのまま組み込めます。",c_ase:"1× の PNG を開いてピクセル単位でリタッチ — 各コンポーネント・ステートが独立したキャンバスで届きます。",c_figma:"SVG は完全編集可能なベクターとして貼り付け — 全パーツ・全レイヤー。どんなカスタムも簡単に再現でき、定番パーツは先にアプリで作って Figma では微調整だけ。",c_penpot:"Penpot は SVG ネイティブ — ベクターは変換されず、そのままあなたのファイルに。パス・グラデーション・グループはすべて元のまま。",c_sketch:"Sketch は SVG 1.1 を編集可能なレイヤーとして読み込み — コンポーネントは命名済みグループで届き、パスもグラデーションもその場で再スタイルできます。",c_aff:"Affinity Designer は SVG 1.1 をカーブそのままに開きます — 全コンポーネントが編集可能。配置だけならステートごとの PNG も。",c_ps:"Photoshop は SVG を読み込み時にラスタライズ — スマートオブジェクトとして配置すれば自由に拡大でき、最大 4× の透過 PNG もコンポーネント・ステートごとに用意。",c_ai:"Illustrator は SVG 1.1 をネイティブに読み込み — パス・グラデーション・グループが完全編集可能なまま、コンポーネントとステートごとに命名されて届きます。",c_ae:"AE はステートごとの PNG をそのままモーションへ。本物のシェイプレイヤーが必要なら、SVG を Illustrator 経由で変換すればきれいに移行できます。",c_blender:"Blender は SVG 1.1 をカーブとしてインポート — UI を押し出して 3D パネルに、または鮮明な PNG をテクスチャに。",c_krita:"Krita は SVG 1.1 をベクターレイヤーで開きます — パスをその場で編集、または PNG 書き出しの上にペイント。",c_web:"Web はホームグラウンド — インライン SVG に、クリックできる実物のコンポーネントを収めた自己完結型 HTML ファイル 1 つ。開いて、調べて、CSS の流用も。",ks1:"ボタン",ks2:"インターフェース",ks3:"コントロール",ks4:"メーターとタイマー",ks5:"HUDとプレイヤー",ks6:"ラージピース",ks7:"エンジンの新パーツ",
