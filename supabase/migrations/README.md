...
| 0095 test-kit bucket | private shelf for the blessed Unity evaluation zip (reads via /api/test-kit, swaps via /api/admin) | **outstanding — optional** (the admin desk's first upload creates the bucket lazily; run this for the record or on a fresh environment) |
